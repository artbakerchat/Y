...
async function ask(message, palette = [], pendingPrompt = '', requestsRemaining = dailyRequestLimit, profile = getAgentProfile('forge')) {
  ...
  if (strands?.Agent) {
    const agent = new strands.Agent({
      systemPrompt: `${CONVERSATION_GUIDANCE} You are ${profile.name}. ${profile.systemPrompt} Talk to the user like a thoughtful, helpful person. Use plain language, keep replies natural and concise, and ask a gentle follow-up when it would help. The user’s current word palette is: ${palette.length ? palette.join(', ') : '(empty)'}. Use palette words as inspiration when relevant, but never invent palette entries or present guesses as facts.`,
    });
    let result;
  ...
  return { answer: cleanAssistantResponse(await askWithBedrock(message, palette, profile)), agent: false };
}
