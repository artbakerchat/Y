...
const htmlRoutes = { '/': 'site/dist/index.html' };
const legacyHtmlAssets = { '/index.html': 'site/dist/index.html' };
const canonicalHtmlRoutes = { '/index.html': '/' };
...
const emptyState = (paletteId = 'default', paletteStory = '') => {
  ...
  const rate = { day: new Date().toISOString().slice(0, 10), count: 0 };
  return {
    agentId: 'forge',
    agentMode: 'auto',
    paletteId,
...
function sessionIdFrom(req) { const match = (req.headers.cookie || '').match(/(?:^|;\s*)larboard_session=([^;]+)/); return /^[a-f0-9-]{36}$/.test(match?.[1] || '') ? match[1] : crypto.randomUUID(); }
function cleanState(value) {
  ...
  const rates = Object.fromEntries(Object.entries(value?.rates || {})
    .filter(([id]) => getAgentProfile(id))
    .map(([id, rate]) => [id, {
  ...
  return {
    ...
    palette: Array.isArray(value?.palette) ? value.palette.filter((word) => typeof word === 'string').map((word) => word.trim()).filter(Boolean).slice(0, 52) : getPaletteTemplate(value?.paletteId || 'default').words.slice(0, 52),
    promptWords: Array.isArray(value?.promptWords) ? value.promptWords.filter((word) => typeof word === 'string').slice(0, 52) : [],
    ...
    rate: rates[agentId],
    rates,
    expiresAt: Number(value?.expiresAt) || Date.now() + stateTtlMs,
  };
}
...
async function ask(message, palette = [], pendingPrompt = '', history = [], profile = getAgentProfile('forge')) {
  const prompt = pendingPrompt ? `Original request: ${pendingPrompt}\nUser clarification: ${message}` : message;
  const result = await runAgent({ agentId: profile.id, prompt, palette, history });
  return { ...result, answer: cleanAssistantResponse(result.answer) };
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && req.url === '/api/ask') { const payload = await body(req); const message = typeof payload?.prompt === 'string' ? payload.prompt : payload?.message; if (typeof message !== 'string' || !message.trim()) return send(res, 400, { error: 'Prompt is required.' }); if (message.length > 4000) return send(res, 413, { error: 'Prompt is too long.' }); if (requestWordCount(message) > maxRequestWords) return send(res, 413, { error: `Requests are limited to ${maxRequestWords} words.` }); if (hasOversizedWord(message)) return send(res, 413, { error: `Each word is limited to ${maxWordCharacters} characters.` }); detectionHints.message = message; const state = await loadState(sessionId, detectionHints); const explicitAgentId = payload?.agent || req.headers['x-agent-id']; const agentId = explicitAgentId || (state.agentMode === 'manual' ? state.agentId : inferAgentId(message, state.agentId)); const profile = getAgentProfile(agentId); if (!profile) return send(res, 400, { error: 'Unknown agent.' }); if (state.agentId !== profile.id) { state.agentId = profile.id; state.messages = []; state.pendingPrompt = ''; } const today = new Date().toISOString().slice(0, 10); if (!state.rates[profile.id] || state.rates[profile.id].day !== today) state.rates[profile.id] = { day: today, count: 0 }; state.rate = state.rates[profile.id]; if (state.rate.count >= profile.dailyRequestLimit) { res.writeHead(429, { ...sessionHeaders(sessionId), 'Retry-After': String(86400 - Math.floor((Date.now() - new Date(`${today}T00:00:00Z`).getTime()) / 1000)) }); return res.end(JSON.stringify({ error: 'Daily request limit reached. Please try again tomorrow.', limit: profile.dailyRequestLimit })); } const prompt = message.trim(); const pendingPrompt = state.pendingPrompt; const answer = await ask(prompt, state.palette, pendingPrompt, state.messages, profile); if (answer.clarification) state.pendingPrompt = prompt; else state.pendingPrompt = ''; state.rates[profile.id].count += 1; state.rate = state.rates[profile.id]; state.messages = [...state.messages, { role: 'user', content: prompt, createdAt: new Date().toISOString() }, { role: 'assistant', content: answer.answer, createdAt: new Date().toISOString() }]; await saveState(sessionId, state); res.writeHead(200, sessionHeaders(sessionId)); return res.end(JSON.stringify({ ...answer, agentId: profile.id })); }
    ...
    const asset = htmlRoutes[pathname] || legacyHtmlAssets[pathname] || `site/dist/${requested}`;
