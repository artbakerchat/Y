...
Open 01-agent-loop/module-01-agent-loop-tools.ipynb in Code Editor. If prompted, click Trust, then select the repository's Python environment. Use the same environment in every module - otherwise cells fail with ModuleNotFoundError: No module named 'strands'. See Setup for details.
